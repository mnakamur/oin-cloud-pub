...
async function getUrl(bunresult) {
	let expires = 604800;
	const pathName =
		"protected/" + bunresult.identityId + "/" + bunresult.createUser;
...
async function sendMail(shomeiItem, request, bunsyoResult, bunSig) {
	...
	const messageBody = `  ${createUser}様からの文書「${docName}」の署名依頼メールが届いています。
  Received a signature request email regarding the document “${docName}” from ${createUser}.\n\n
  リンク: ${link} `;
...
-------------------------------------- `;
	...
	const mailParams = {
		...
		Message: {
			...
			Subject: {
				Data: `Signature request:${createUser}様の文書「${docName}」に関する署名依頼`,
			},
		},
		Source: sourceHeader,
