...
const {
	...
	_signerToAsn1,
	_attributeToAsn1,
} = require("node-signpdf/src/asn1util");
...
exports.handler = async (event) => {
	...
	const pathName =
		"protected" + "/" + eventBody.s3folder + "/" + eventBody.createUserId;
...
async function signFunc(pdfBuffer, p12, pemCert, shomeiName, widgetItem) {
	pdfBuffer = await plainAddPlaceholder({
		pdfBuffer,
		reason: shomeiName + "によって署名されました:signed",
	...
	var { p7, pdf, byteRange, placeholderLength } = signer_md.sign(
		pdfBuffer,
		p12,
		pemCert
	);
...
const parseRequestBody = (event) =>
	event.httpMethod === "POST" ? JSON.parse(event) : event;
