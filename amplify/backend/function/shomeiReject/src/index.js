...
exports.handler = async (event) => {
	...
	try {
		...
		const commentToAuthor =
			shomeiR["shomeiItems"][requestBody.route].commentToAuthor;
...
async function mailSend(
	...
	try {
		...
		const fixedText = `  文書${docName}が${rejectMailer}様に却下されました。\n　下記リンクより確認してください。\n\n
    ${docName} was rejected by ${rejectMailer}.\n Please check msg from following link`;
		const signature = `
    ...
    ${signature} `;
		var mail_params = {
			...
			Message: {
				...
				Subject: {
					Data: `Doc was rejected:文書${docName}への署名が却下されました`,
				},
			},
			Source: sourceHeader,
