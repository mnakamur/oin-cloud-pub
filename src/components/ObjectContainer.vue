<script setup>
	...
	const formattedHTML = ref(t('↓↑で行数と     ←→で文字領域を調整してください'));
	...
	function handleBlur_signdo() {
	  ...
	  const byteLengths = lines.map(line => encoder.encode(line).length)
	  const maxByteLength = Math.max(...byteLengths)
	  //const lineLengths = lines.map((line) => line.length);
	  //const maxCharLength = Math.max(...lineLengths);
	  const langSize = locale.value === 'ja' ? 2.2 : 1.9;
	  const fontsize = Math.floor(Math.min(props.width / (maxByteLength/langSize), props.height / lines.length));
		...
		>
          ...
          width: textAreaWidth + 'px',
			...
			<input
				...
				maxlength="60"
				rows="1"
				:placeholder="t('入力説明文を40字以内で記入してください')"
				@blur="handleBlur_C"
	...
	<div v-else-if="origin == 'signdo'">
		...
		>
          ...
          borderColor: 'transparent',
			...
			/>
			<p v-if="operation == 'input'" class="objComment_date">
				{{ objComment }}
			</p>
		</div>
		...
		>
			...
			<div v-if="type == 'image'" class="button-container">
				...
				<input
					...
					@change="uploadImage"
				/>
				<p v-if="operation == 'input'" class="objComment_image">
					{{ objComment }}
				</p>
			</div>
			...
			/>
			<p
				v-if="type === 'drawing' && operation === 'input'"
				class="objComment_draw"
			>
				{{ objComment }}
			</p>
		</div>
