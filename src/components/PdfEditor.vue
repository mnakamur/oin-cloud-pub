...
<template>
	<div style="margin-bottom: 4px; position: relative">
		<div v-if="!userProfile" style="color: red">
			{{$t('プロフィールが設定されていません.事前に設定してください')}}
		</div>
		<div id="dragArea" class="dragArea">
			<div>
				...
				<a
					class="htmlLink"
					href="/guide/oin-cloudDoc.html#pdfspec"
					target="_blank"
					rel="noopener"
					>{{$t('登録できるPDF文書')}}</a
				>
			...
			<div v-if="processStep > 1">
				<button class="buttonR" @click="makeRoot">
					{{$t('文書を保存して署名ルートを設定します。')}}
				</button>
			</div>
		</div>
		<div class="docArea">
			<div
				v-if="processStep > 1"
				style="width: 100%; overflow: scroll; margin-top: -20px"
			>
				{{$t('対象文書')}}
				<div
