...
<template>
  <div class="headexplain">
    ...
    <p>
      <strong
        >電子署名された文書はダウンロードして手元に保存してください。<br />この文書は7日後には参照できなくなります。</strong
      >
    </p>
    <h4>文書名：{{ docData.docName }}</h4>
    <h4>送信者：{{ createUserName }}</h4>
  </div>
  <div class="opearea">
    <button v-if="dlButtonStatus" class="buttonB" @click="handlePdfDownload">
      署名後の文書をダウンロードします
    </button>
    <button class="button" @click="closeWindow">画面クローズ</button>
  </div>
  <div class="docarea">
    <h4>署名文書</h4>
    <div v-for="(page, pageIndex) in docData.pages" :key="pageIndex" class="selected-page">
