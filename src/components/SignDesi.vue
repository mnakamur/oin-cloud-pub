	...
	<div class="main">
		...
		<div v-if="processStep == 1" style="margin-bottom: 5px">
			<h4 style="margin: 5px auto">
				{{$t('署名時の入力する領域を設定します。')}}
				<button @click="showModal">{{$t('操作説明を見る')}}</button>
			</h4>
			<ExplanationSigndesi
				v-if="isModalVisible"
		...
		<div id="dragArea" class="dragArea">
			<div v-if="processStep == 1" style="margin-top: 10px">
				<button class="buttonR" @click="contentsSave">
					{{$t('署名設定を保存する')}}
				</button>
			</div>
			<div v-if="processStep == 1" style="margin-top: 50px; float: left">
				<button @click="contentsClose">{{$t('入力設定にもどる')}}</button>
			</div>
		</div>
		<div v-if="scaleSize !== 0" id="docArea" class="docArea" ref="docArea">
			<h4>
				{{$t('文書名')}}：{{ data.docName }} <br />{{$t('署名者')
			...
			>
				...
				>
					<pdf-page
						...
						@add-date="addDateElement('署名日付を入力してください', 100, 332)"
						@add-draw="addDrawObj"
