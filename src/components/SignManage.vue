<script setup>
	...
	import { convertForDisplay,convertForDisplayE, formatToJST, useMsgHandler } from './utils/helper';
	...
	{ items.push({
	      ...
	      route: item.shomeiLength ,
	      status: convertForDisplayE(item.bunStatus),
	      time: item.updatedAt,
	    });
	}
	  });
...
<template>
	<div>
		<h4>
			作成者：{{ createUser }}
			<button class="profile" @click="profile">
				{{$t('プロフィール編集')}}
			</button>
		</h4>
		<router-link
	...
	<div>
		...
		<table class="table">
			<thead>
				<tr>
					<th scope="col" />
					<th scope="col">{{$t('文書名')}}</th>
					<th scope="col">{{$t('作成日')}}</th>
					<th scope="col">{{$t('ルート数')}}</th>
					<th scope="col">{{$t('状態')}}</th>
					<th scope="col">{{$t('最終更新時間')}}</th>
				</tr>
			</thead>
			<tbody>
				<tr v-for="(item, index) in items" :key="index">
					...
					<td>
						<a href="#" @click.prevent="pdfmshow(item.docId, item.docName)">
							{{ item.docName }}</a
						>
					</td>
					...
					<td>
						<a
							href="#"
							@click.prevent="rootshow(item.docId)"
							>{{ item.route }}</a
						>
					</td>
					<td>{{ item.status }}</td>
