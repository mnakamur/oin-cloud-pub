<script setup>
	...
	const validateForm = () => {
	  ...
	  rootItems.forEach((item, index) => {
	    ...
	    if (!item.name.trim()) {
	      itemErrors.name = t('名前は必須です');
	    } else if (!validateName(item.name)) {
	      itemErrors.name = t('名前に不正な文字が含まれています。修正してください');
	    }
	    if (!item.email.trim()) {
	      itemErrors.email = t('Eメールは必須です');
	    } else if (!validateEmail(item.email)) {
	      itemErrors.email = t('有効なEメールアドレスを入力してください');
	    }
	...
	function closeWindow() {
	  const userConfirmed = window.confirm(t('画面を終了して初期画面に戻ります'));
	  if (userConfirmed) {
...
<template>
	...
	<div style="margin-top: -35px">
		<h3 style="margin-bottom: 5px">{{$t('署名ルート作成')}}</h3>
		<div v-if="processStep == 1" style="margin-bottom: 5px">
			<h4>
				{{$t('文書を回送するルートを作成します。')}}<br />
				{{$t('ルートを追加し、メールアドレスを入力します。')}}<br />
				{{$t('各署名者の属性を選択します。署名時に入力（押印、日付等の入力）がある場合は、署名入力ありを選択してください。')}}
			</h4>
		</div>
		<h4
			v-if="processStep > 1 && signInputFlg == true"
			style="margin-bottom: 5px"
		>
			{{$t('入力設定ボタンを押して入力設定を行ってください。設定完了後、文書を回送してください')}}
		</h4>
		<h4
			v-if="processStep > 1 && signInputFlg == false"
			style="margin-bottom: 5px"
		>
			{{$t('文書を回送します')}}
		</h4>
		<h4 :style="signdesi.shomeiSend ? { color: 'blue', fontSize: '20px' } : {}">
			{{$t('文書名')}}：{{ desiData.docName }}
		</h4>
	</div>
	<Suspense>
		<div id="app">
					...
					<tbody>
						<tr v-for="(item, index) in rootItems" :key="index">
							...
							<td>
								...
								>
									<option value="signonly">
										{{$t('署名のみ（入力なし）')}}
									</option>
									<option value="signinput">{{$t('署名入力あり')}}</option>
								</select>
							</td>
							<td>
								<button
								...
								<div v-if="item.status === 'true'">{{$t('設定完了')}}</div>
							</td>
						</tr>
					</tbody>
				</table>
        ...
        "
			...
			<button
				type="button"
				v-if="signdesi.shomeiSend && processStep > 1"
				@click="closeWindow"
			>
				{{$t('画面クローズ')}}
			</button>
