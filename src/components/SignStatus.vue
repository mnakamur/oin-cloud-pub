<script setup>
	...
	import { convertForDisplay, convertForDisplayE,formatToJST } from './utils/helper';
	...
	else
	{
	    ...
	    const shomeiItem = {
	      ...
	      name: item.name,
	      email: item.mail,
	      attribute: item.shomeiNaiyo,
	      status: convertForDisplayE(item.shomeiStatus),
	      time: item.updatedAt,
	      message: item.commentToAuthor,
	    };
	    if (props.origin == 'signdo' && shomeiItem.status === 'doc save') {
	      shomeiItem.status = '';
	      shomeiItem.time = '';
	    }
	    formItems.push(shomeiItem);
...
<template>
	<div class="rootshow">
		<div style="margin-left: 5px">
			<h4 style="margin-bottom: 3px">{{$t('署名ステータス参照')}}</h4>
			<h5 v-if="fullShow" style="margin: 1px">
				{{$t('文書のステータスを確認します。')}}
				{{$t('却下されている場合は理由を確認して、再度文書登録から実施してください。')}}
			</h5>

			<h4 style="margin: 1px">{{$t('文書名')}}：{{ bunsyoName }}</h4>
		</div>

		<table>
			<thead>
				<tr>
					<th rowspan="2">{{$t('順序')}}</th>
					<th>{{$t('名前')}}</th>
					<th v-if="fullShow">e-mail</th>
					<th v-if="fullShow">{{$t('属性')}}</th>
					<th>{{$t('状態')}}</th>
					<th v-if="fullShow">{{$t('処理時刻')}}</th>
				</tr>
				<tr>
					<th v-if="fullShow" colspan="5">{{$t('メッセージ')}}</th>
				</tr>
			</thead>
			<tbody v-for="(item, index) in formItems" :key="index">
				<tr
					:style="{ backgroundColor: parseInt(props.routeNo) === index ? '#cce7ff' : '' }"
				>
					<td rowspan="2">
						{{ index + 1 }}
					</td>
					<td>
						{{ item.name }}<span style="float: right">{{$t('様')}}</span>
					</td>
					<td v-if="fullShow">
						{{ item.email }}
					</td>
		...
		<button v-if="fullShow" @click="goEditor">{{$t('作成画面に戻る')}}</button>
		<button v-if="origin == 'manage'" @click="closeStatus">
			{{$t('管理画面に戻る')}}
		</button>
		<button v-else-if="origin == 'signdo'" @click="closeStatus">
			{{$t('署名画面に戻る')}}
		</button>
		<button v-else @click="goEditor">
			{{$t('作成画面で詳細を確認する')}}
		</button>
