...
<template>
	...
	>
		<div
			v-if="objtype == 'drawing' ||objtype == 'txt'  "
		...
		>
			...
			<button
				class="pm-button w-1 left-0 bottom-0"
				@click="$emit('sizehminus')"
			>
				↑
			</button>
			<button
				class="pm-button w-1 left-0 bottom-0"
				@click="$emit('sizelminus')"
			>
				←
			</button>
			...
			<select
				...
				class="font-select"
			>
				<option value="ipaexg">{{$t('IPAゴシック')}}</option>
				<option value="ipaexm,serif">{{$t('IPA 明朝')}}</option>
