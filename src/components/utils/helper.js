...
export function convertForDisplay(int_code) {
	...
	if (int_code.includes(",")) {
		...
		return `${Number(signedStatus[1]) + 1}人目 ` + convertMap[signedStatus[0]];
	} else {
		return convertMap[int_code] || int_code;
	}
}
export function convertForDisplayE(int_code) {
	const convertMap = {
		create: "doc create",
		save: "doc save",
		send: "sending",
	...
	if (int_code.includes(",")) {
		...
		return (
			"Member " +
			`${Number(signedStatus[1]) + 1} ` +
			convertMap[signedStatus[0]]
		);
	} else {
		return convertMap[int_code] || int_code;
	}
