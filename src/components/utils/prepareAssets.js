...
export const Fonts = {
    ...fonts,
    ipaexg: {
        src: 'src/fonts/ipagp.ttf', // 9.9 MB
        correction(size, lineHeight) {
          return (size * lineHeight - size) / 2;
        },
      },
      ipaexm: {
        src: 'src/fonts/ipamp.ttf', // 9.9 MB
        correction(size, lineHeight) {
          return (size * lineHeight - size) / 2;
        },
      },
    ...
    fonts[name] = fetch(font.src)
        ...
        .then((fontBuffer) => {
            ...
            fontFace.load().then(() => {document.fonts.add(fontFace)
                           ...
                           .catch((error) => {
                            console.error(`Font '${name}' load error:`, error);
                            });
            return {
                ...font,
                buffer: fontBuffer,
